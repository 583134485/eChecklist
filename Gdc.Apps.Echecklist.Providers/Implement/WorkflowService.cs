...
namespace Gdc.Apps.Echecklist.Providers.Implement
{
    public  class WorkflowService:IWorkflowService
    {
        ...
        public ViewData<Workflow> GetOneWorkflow(Workflow workflow)
        {
            ViewData<Workflow> viewdata = new ViewData<Workflow>();
            ...
            {
                Workflow result = workflowRepositories.GetOneByName(workflow.Name);
                if (result != null)
                {
                    viewdata.StatusCode = SUCCESS;
                    viewdata.SetData(result);
        ...
        public ViewData<Workflow> InsertWorkflow(Workflow workflow)
        {
            ViewData<Workflow> viewData = new ViewData<Workflow>();
            Workflow ifworkflow = workflowRepositories.GetOneByName(workflow.Name);
            if (ifworkflow != null)
            {
                viewData.StatusCode = REPETITION;
                return viewData;
            }
            bool result = workflowRepositories.Insert(workflow);
            if (result)
            {
                Workflow reworkflow = workflowRepositories.GetOneByName(workflow.Name);
                if (reworkflow != null)
                {
        ...
        public ViewData<Workflow> UpdateWorkflow(Workflow workflow)
        {
            ViewData<Workflow> viewData = new ViewData<Workflow>();
            if (workflow.Id == null)
            {
        ...
        public ViewData<Workflow> RemoveWorkflow(Workflow workflow)
        {
            ViewData<Workflow> viewData = new ViewData<Workflow>();
            if (workflow.Id == null)
            {
