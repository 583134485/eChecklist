   ...
   public  class StepRepositories : IStepRepositories
    {
        ...
        {
          // this.db.GetCollection<Step>(Collection).UpdateOne(step.Id,step.WorkflowID);
        ...
        {
            return this.db.GetCollection<Step>(Collection).Find(x => x.WorkflowID == id).ToEnumerable().ToList();
            // throw new NotImplementedException();
        }
        public Step GetOneStepByNameAndWorkflowId(Step step)
        {
            
            return this.db.GetCollection<Step>(Collection).AsQueryable().Where(x => x.Name == step.Name).Where(x => x.WorkflowID == step.WorkflowID).FirstOrDefault();
            //return this.db.GetCollection<Step>(Collection).Find(x => x.Name == step.Name).FirstOrDefault();
