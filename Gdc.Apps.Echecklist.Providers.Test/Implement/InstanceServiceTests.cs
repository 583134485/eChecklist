...
namespace Gdc.Apps.Echecklist.Providers.Implement.Tests
{
    ...
    {
       readonly Mock<IInstanceRepositories> InstanceDaoMock = new Mock<IInstanceRepositories>();

       readonly Mock<ITemplateRepositories> TemplateDaoMock = new Mock<ITemplateRepositories>();

        //2 new "Instance"dd d test dd dd dd ss s case
        readonly Instance ins1 = new Instance();
        readonly Instance ins2 = new Instance();
         Instance ins3 = new Instance();
        readonly Template temp = new Template();
        Template temp_ = new Template();
        readonly List<Instance> instances = new List<Instance>();
        //view case
        readonly  InstanceView view = new InstanceView();
        ...
        [Test()]
        public void InsertBatchInsertTest()
        {
            Assert.Pass();
        }

        [Test()]
        public void InsertInstanceForStepTest()
        {
            InitParamter();
         //   List<String> list = new mock(List.class); 
           // InstanceDaoMock.Setup(x => x.GetSomeInstanceByStepId(ins1))
          // InstanceDaoMock.When()
        }
    }
}